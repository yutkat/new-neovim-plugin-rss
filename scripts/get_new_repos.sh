...
cat /tmp/repo.json | jq -r '.items[] |
  select(.full_name | test("/nvim-") or endswith(".nvim")) |
  select(.full_name | test("/nvim-config") or test("/nvim-conf") or endswith("/nvim") | not) |
  select(.language != "Vim Script") |
  select(contains({description: "colorscheme"}) or contains({description: "config"}) | not) |
  select(.topics | index("neovim-colorscheme") or index("colorscheme") or index("dotfiles") | not)
